...
def Train(conc_array):
    ...
    history = model.fit(train_data, train_labels, epochs=300, validation_data=(validation_data, validation_labels),
                        batch_size=16)
    model.save('output/' + name + '_five_finger_model.h5')
    ...
    plt.savefig('output/%s_acc.jpg' % name)
    ...
    plt.xlabel('epoch')
    plt.legend(['train', 'test'], loc='upper left')
    plt.savefig('output/%s_loss.jpg' % name)
...
def main():
    ...
    conc_array = np.concatenate(
        [thumb_open_averages, index_open_averages, middle_open_averages, ring_open_averages, pinky_open_averages],
        axis=0)
...
def my_train():
    conc_array = np.loadtxt('output/aaa_five_movements.txt')
    Train(conc_array)
